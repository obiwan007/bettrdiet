...
namespace BettrFit.Core.Common
{
    public static class CultureHelper
    {
        ...
        {
            return GetNeutralCulture(CultureInfo.CurrentUICulture.Name);
        }
        ...
        public static string GetLocalString(string txt)
        {
            if (txt == null)
                return "Error:null";
            if (txt.Contains("|"))
            {
                ...
                int index = 0;
                if (CultureHelper.GetCurrentCulture().StartsWith("en"))
                {
                    index = 0;
                }
                else if (GetCurrentCulture().StartsWith("de"))
                {
                    index = 1;
                }
                if (a[index].Length == 0)
